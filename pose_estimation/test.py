...
def _non_white_mask(rgb_img: torch.Tensor, threshold: float = 0.95) -> torch.BoolTensor:
    """Return foreground mask by removing near-white background."""
    return ~((rgb_img > threshold).all(dim=-1))
...
def test_pose_estimation(
    ...
    for img_idx, camera_info in tqdm(enumerate(cameras_info)):
        ...
        if obs_img.shape[-1] == 4:
            ...
            white_thr = float(os.environ.get("POSE_WHITE_BG_THRESHOLD", "0.95"))
            mask_img = _non_white_mask(obs_img[..., :3], threshold=white_thr)
